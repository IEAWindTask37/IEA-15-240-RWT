# -*- coding: utf-8 -*-
"""Make the HAWC2 monopile st file from the yaml. Also make a version fixed at the mudline.
...
body = 'monopile'  # white body to get from the yaml file
save_st = 1  # save the HAWC2 st file?

# paths
mydir = os.path.dirname(os.path.realpath(__file__))  # get path to this file
yaml_path = os.path.join(mydir,'../../../WT_Ontology/IEA-15-240-RWT.yaml')  # yaml file with data
# ed_path = os.path.join(mydir,'../../../OpenFAST/IEA-15-240-RWT-Monopile/IEA-15-240-RWT-Monopile_ElastoDyn_tower.dat')  # !!! would need to compare with SubDyn file
h2_st_path = os.path.join(mydir,'../data/IEA_15MW_RWT_Monopile_st.dat')  # file to write for HAWC2 model
...
# get dimensions
stations = np.array(res['components'][body]['outer_shape_bem']['reference_axis']['z']['values'])
stations -= stations[0]
...
out_diam = out_diam[1:]
t = t[1:]
#   4. Manually remove -74.999
stations = np.delete(stations, [1])
out_diam = np.delete(out_diam, [1])
t = np.delete(t, [1])

print('s [m]  OD [m]    t [mm]')
for i in range(t.size):
    print(f'{stations[i]:8.3f}   {out_diam[i]:4.1f}m   {t[i]*1000:5.1f}mm')

# Visualize the outer diameter and wall thickness
plt.figure(1, figsize=(7, 3))
plt.clf()
plt.subplot(1, 2, 1)
plt.plot(out_diam, stations)
plt.xlabel('Outer diameter [m]'); plt.ylabel('Tower height [m]'); plt.grid('on')
plt.subplot(1, 2, 2)
plt.plot(t*1000, stations)
...
df['Height [m]'] -= df.loc[0, 'Height [m]']
if not np.allclose(df['Height [m]'], stations):
