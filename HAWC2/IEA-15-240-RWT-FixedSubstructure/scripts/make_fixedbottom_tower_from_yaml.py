...
G *= 1e7
print('Assuming tower is torsionally rigid!')
...
if save_twr:
    ...
    header = '\n'.join([header1, header2, f'$1 {out_arr.shape[0]} Flexibile (no torsion)'])
    fmt = ['%.3f'] + ['%.4E'] * 18
    np.savetxt(h2_st_path, out_arr, delimiter='\t', fmt=fmt, header=header, comments='')
    # rigid
    out_arr[:, 8] *= 1e7
    header = '\n'.join([header2, f'$2 {out_arr.shape[0]} Rigid'])
